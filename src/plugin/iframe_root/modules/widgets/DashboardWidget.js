define([
    ...
    'kb_common/gravatar',
    'module'
], function (nunjucks, $, Promise, html, Utils, ServiceApi, NarrativeMethodStore, Logger, gravatar, module) {
    'use strict';
    var DashboardWidget = Object.create(
        {},
        {
                ...
                value: function (cfg) {
                    ...
                    var loaders = [
                        new nunjucks.WebLoader(
                            this.runtime.pluginPath + '/resources/' + this.widgetName + '/templates',
                            true
                        ),
                        new nunjucks.WebLoader(this.runtime.pluginPath + '/resources/DashboardWidget/templates', true)
                    ];
                    this.templates.env = new nunjucks.Environment(loaders, {
                        autoescape: true
                    });
                    this.templates.env.addFilter(
                        'roleLabel',
                        function (role) {
                    ...
                    this.templates.env.addFilter(
                        'gravatar',
                        function (email, size, rating, gdefault) {
                            // TODO: http/https.
                    ...
                    this.context = {};
                    this.context.env = {
                        pluginPath: this.runtime.pluginPath + '/resources/',
            ...
            setupCoreApp: {
                value: function () {
                    ...
                    // to synchronize or just plain rebuild.
            ...
            getApps: {
                value: function () {
                    var methodStore = new NarrativeMethodStore(
                        ...
                        tag = this.getTag();
                    return Promise.all([methodStore.list_methods({ tag: tag })]).spread(
                        function (release) {
                            var appMap = {};
                    ...
                    return Promise.all([
                        ...
                        .spread(
                            function (narratives, appsMap) {
                                ...
                                narratives.forEach(
                                    function (narrative) {
                                        narrative.apps = narrative.apps.map(
                                            function (app) {
                                                if (!app.module) {
                                                    return {
                                                        name: app.id,
         ...
         */
            ...
            // loadCSS: {
            //     value: function () {
            //         this.loadCSSResource(this.pluginPath + '/resources/' + this.widgetName + '/css/style.css');
            //     }
            // },
            renderMessages: {
                value: function () {
